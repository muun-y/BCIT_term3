...
class CityDatabase:
    ...
    def __init__(self, file_path):
        ...
        """
        #
        self.city_db = [CityInfo(row[1]["city_ascii"], row[1]["lat"], row[1]["lng"])
...
class CityOverheadTimes:
    ...
    def __init__(self, city: CityInfo, *args):
        """
...
class ISSDataRequest:
    ...
    @classmethod
    async def get_overhead_pass(cls, city: CityInfo) -> CityOverheadTimes:
        # pass
        # Write request code here!
        # print(city)
        # response = requests.get(
        # cls.OPEN_NOTIFY_OVERHEAD_PASS_URL, params={"lat": city.lat, "lon": city.lng}
        # )
        # data = response.json()
        # print(response)
        data = {
